...
INDEX_TEMPLATE_ARGUMENTS
thread_local bool BPlusTree<KeyType, ValueType, KeyComparator>::root_is_locked =
    false;
...
bool BPLUSTREE_TYPE::InsertIntoLeaf(const KeyType &key, const ValueType &value,
  ...
  if (leaf->GetSize() < leaf->GetMaxSize()) {
    ...
    // be equal.
    auto *leaf2 =
        Split<BPlusTreeLeafPage<KeyType, ValueType, KeyComparator>>(leaf);
...
void BPLUSTREE_TYPE::InsertIntoParent(BPlusTreePage *old_node,
  ...
  if (old_node->IsRootPage()) {
    ...
    auto root = reinterpret_cast<BPInternalPage *>(page->GetData());
    ...
    auto internal = reinterpret_cast<BPInternalPage *>(page->GetData());
    ...
    if (internal->GetSize() < internal->GetMaxSize()) {
      ...
      auto *copy = reinterpret_cast<BPInternalPage *>(page2->GetData());
      copy->Init(page_id);
...
void BPLUSTREE_TYPE::Coalesce(N *&neighbor_node, N *&node,
                              BPInternalPage *&parent, int index,
                              Transaction *transaction) {
...
void BPLUSTREE_TYPE::Redistribute(N *neighbor_node, N *node, int index) {
  if (index == 0) {
    ...
    auto parent = reinterpret_cast<BPInternalPage *>(page->GetData());
...
bool BPLUSTREE_TYPE::AdjustRoot(BPlusTreePage *old_root_node) {
  ...
  if (old_root_node->GetSize() == 1) {
    auto root = reinterpret_cast<BPInternalPage *>(old_root_node);
    ...
    auto new_root = reinterpret_cast<BPInternalPage *>(page->GetData());
...
void BPlusTree<KeyType, ValueType, KeyComparator>::UnlockUnpinPages(
...
// leaf node and internal node may have different MAXSIZE
INDEX_TEMPLATE_ARGUMENTS
template <class N>
bool BPlusTree<KeyType, ValueType, KeyComparator>::isSafe(N *node,
                                                          Operation op) {
...
BPLUSTREE_TYPE::FindLeafPage(const KeyType &key, bool leftMost, Operation op,
                             Transaction *transaction) {
  if (op != Operation::READONLY) {
    lockRoot();
    root_is_locked = true;
  ...
  while (!node->IsLeafPage()) {
    auto internal = reinterpret_cast<BPInternalPage *>(node);
